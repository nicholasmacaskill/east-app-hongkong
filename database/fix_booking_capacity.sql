-- Migration: Final Cleanup and Update for Master Booking Logic
-- This version drops the old overloaded signatures to ensure the RPC call is unambiguous.

-- 1. Drop known old signatures
DROP FUNCTION IF EXISTS public.master_book_atomic(uuid, bigint, uuid[], uuid, text, text);
DROP FUNCTION IF EXISTS public.master_book_atomic(uuid, bigint, uuid[], uuid, text, text, uuid);

-- 2. Create the fixed version (7 arguments)
...
BEGIN
  ...
  -- 7. CHECK CAPACITY
  SELECT count(*) INTO v_current_bookings FROM public.registrations 
  WHERE session_id = p_session_id AND (status IS NULL OR status != 'cancelled');

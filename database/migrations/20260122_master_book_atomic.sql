-- Migration: Master Booking Atomicity (Bug #11)
-- This RPC unifies facility and coach bookings into a single atomic transaction.
...
BEGIN
  -- 1. LOCK USER PROFILE for update to prevent concurrent credit issues
  SELECT 
  ...
  -- 2. HANDLE CHILD ACCOUNTS (Check parent's status)
  ...
  -- Total cost = (Main Cost * Count) + (Coach Cost * Count if coach selected)
  v_total_cost := 0;
  ...
  -- 7. CHECK CAPACITY (Advisory lock or row lock on sessions)
  ...
  IF p_coach_id IS NOT NULL THEN
    ...
    -- Fetch coach info for session creation
    ...
    -- Create Private Session for the Coach
  ...
  FOREACH v_attendee_id IN ARRAY p_attendee_ids LOOP
    -- Book Facility
    IF p_origin = 'facilities' THEN
      INSERT INTO public.registrations (session_id, user_id, attendee_id, credits_paid)
      VALUES (p_session_id, p_user_id, v_attendee_id, v_main_session_cost);
    ...
    IF v_coach_session_id IS NOT NULL THEN
      INSERT INTO public.registrations (session_id, user_id, attendee_id, credits_paid)
      VALUES (v_coach_session_id, p_user_id, v_attendee_id, v_coach_cost);

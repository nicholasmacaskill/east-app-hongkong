...
BEGIN
  ...
  -- FIX: If origin is facilities OR (coaches but no specific coach requested), charge main session cost
  ...
  FOREACH v_attendee_id IN ARRAY p_attendee_ids LOOP
    ...
    -- FIX: Execute if facilities OR (coaches but no specific coach ID -> standard booking)
    IF p_origin = 'facilities' OR (p_origin = 'coaches' AND p_coach_id IS NULL) THEN

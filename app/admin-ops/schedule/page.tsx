            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black italic uppercase tracking-tighter mb-2">
                        Master <span className="text-east-light">Schedule</span>
                    </h1>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={fetchSchedule} className="p-4 bg-[#1a1a1a] rounded-2xl hover:bg-white/5 transition-colors border border-white/5 group">
                        <RefreshCw size={22} className={loading ? 'animate-spin text-east-light' : 'text-gray-600 group-hover:text-east-light'} />
                    </button>
                </div>
            </div>
                    ...
                    {weekDays.map((day, i) => {
                        ...
                        return (
                            <button key={i} onClick={() => setSelectedDate(dateStr)} className={`flex flex-col items-center justify-center py-5 rounded-3xl transition-all border ${isSelected ? 'bg-east-light border-transparent text-black shadow-xl shadow-[#28D160]/20 scale-105 z-10' : 'bg-black/20 border-white/5 text-gray-500 hover:border-white/20 hover:bg-white/5'}`}>
                                ...
                                {isToday && !isSelected && <div className="mt-2 w-1.5 h-1.5 bg-east-light rounded-full animate-pulse" />}
                            </button>
                        );
                    })}
                </div>
            ...
            <div className="flex flex-col lg:flex-row gap-6 items-center justify-between pb-4">
                <div className="flex overflow-x-auto no-scrollbar gap-2 w-full lg:w-auto p-1">
                    {['ALL', 'PRIVATE', 'FACILITY', 'CLASS', 'EVENT'].map(cat => (
                        <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${activeCategory === cat ? 'bg-east-light border-transparent text-black shadow-lg shadow-[#28D160]/20' : 'bg-[#1a1a1a] border-white/5 text-gray-500 hover:border-east-light/30'}`}>
                            {cat === 'ALL' ? 'Universe' : cat.replace('_', ' ')}
                        </button>
                    ))}
                </div>
                <div className="flex gap-3 w-full lg:w-auto">
                    <select value={filterFacilityId} onChange={(e) => setFilterFacilityId(e.target.value)} className="bg-[#1a1a1a] border border-white/5 text-white text-[10px] font-black uppercase p-4 rounded-xl outline-none focus:border-east-light flex-1 lg:w-56 appearance-none cursor-pointer">
                        <option value="ALL">All Facilities</option>
                        {Array.from(new Set(services.filter(s => s.category === 'FACILITY').map(s => s.title))).map(title => (
                            <option key={title} value={title}>{title}</option>
                        ))}
                    </select>
                    <select value={filterCoachId} onChange={(e) => setFilterCoachId(e.target.value)} className="bg-[#1a1a1a] border border-white/5 text-white text-[10px] font-black uppercase p-4 rounded-xl outline-none focus:border-east-light flex-1 lg:w-56 appearance-none cursor-pointer">
            ...
            <div className="space-y-4">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-32 bg-[#1a1a1a] rounded-[3rem] border border-white/5">
                        <RefreshCw size={56} className="animate-spin mb-6 text-east-light/20" />
                    ...
                    <div className="text-center py-32 bg-[#1a1a1a] rounded-[3rem] border border-dashed border-white/10 group">
                        <Plus size={64} className="mx-auto mb-6 text-gray-800 group-hover:text-east-light transition-colors" />
                        <h3 className="text-2xl font-black italic uppercase text-gray-700">No active deployments</h3>
                        <button onClick={() => handleCellClick("09:00")} className="mt-6 bg-east-light text-black px-10 py-4 rounded-2xl font-black text-xs uppercase italic tracking-widest hover:bg-white transition-all shadow-xl active:scale-95">Initialize Schedule</button>
                    </div>
                ) : (
                                    ...
                                    <div className="flex-1 py-1">
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex items-center gap-3">
                                                ...
                                                {!isSlot && <span className={`${item.category === 'FACILITY' ? 'bg-east-light/10 text-east-light' : 'bg-blue-500/10 text-blue-400'} text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest`}>{item.category}</span>}
                                            ...
                                            <div className="flex flex-col gap-1">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-5 h-5 rounded-lg bg-white/5 flex items-center justify-center">
                                                        <User size={12} className={isSlot ? 'text-gray-800' : 'text-east-light'} />
                                                    </div>
                                                ...
                                                {item.registrations && item.registrations.length > 0 && (
                                                    <div className="flex items-center gap-2 ml-8 mt-1">
                                                        ...
                                                        <span className="text-[10px] text-east-light font-black uppercase italic tracking-widest">{item.registrations.length} Active Duty Athletes</span>
                                                    </div>
                                                )}
                                            </div>
                                            {!isSlot && (
                                                <div className="flex items-center gap-6">
                                                    <div className="text-right">
                                                        ...
                                                        <p className="text-sm font-black italic text-east-light leading-none mt-1">{item.credit_cost}</p>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                ...
                <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-2xl flex items-center justify-center p-4">
                    <div className="bg-[#1a1a1a] p-10 rounded-[3rem] w-full max-w-xl border border-white/10 shadow-[0_0_100px_rgba(40,209,96,0.1)] overflow-y-auto max-h-[90vh] no-scrollbar">
                        <div className="flex justify-between items-center mb-10">
                            <div>
                                <h2 className="font-black italic text-4xl uppercase tracking-tighter text-east-light">{modalAction === 'CREATE' ? 'New Deployment' : 'Review Session'}</h2>
                        ...
                        <div className="space-y-8">
                            <div className="space-y-6 bg-black/40 p-8 rounded-[2rem] border border-white/5 shadow-inner">
                                <div>
                                    <label className="text-[10px] font-black text-east-light uppercase tracking-widest px-2 mb-3 block italic">Template Alignment</label>
                                    ...
                                    }} className="w-full bg-black/80 border border-white/5 p-5 rounded-2xl text-white outline-none focus:border-east-light/50 text-sm font-black italic appearance-none cursor-pointer">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input value={editingSession.title} onChange={e => setEditingSession({ ...editingSession, title: e.target.value })} className="w-full bg-black/80 border border-white/5 p-5 rounded-2xl text-white outline-none focus:border-east-light/50 font-black italic text-sm" />
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-black text-gray-700 uppercase tracking-widest px-2 mb-3 block">Deployment Lead</label>
                                        <select value={editingSession.instructor} onChange={e => setEditingSession({ ...editingSession, instructor: e.target.value })} disabled={editingSession.lockInstructor} className={`w-full bg-black/80 border border-white/5 p-5 rounded-2xl text-xs font-black italic outline-none focus:border-east-light/50 appearance-none cursor-pointer ${editingSession.lockInstructor ? 'opacity-50 grayscale' : ''}`}>
                            ...
                            {modalAction === 'CREATE' && (
                                <div className="bg-black/40 p-8 rounded-[2rem] border border-white/5">
                                    <div className="flex items-center justify-between mb-6">
                                        ...
                                        <button onClick={() => setRecurring(!recurring)} className={`w-12 h-6 rounded-full transition-all relative ${recurring ? 'bg-east-light' : 'bg-white/5 border border-white/10'}`}>
                                ...
                                <div className="bg-black/40 p-8 rounded-[2rem] border border-white/5">
                                    ...
                                    <div className="space-y-3 max-h-[150px] overflow-y-auto no-scrollbar">
                                        {registrations.map(reg => (
                                            <div key={reg.id} className="flex justify-between items-center bg-black/60 p-4 rounded-2xl border border-white/5 transition-hover hover:border-east-light/30">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-xl bg-east-light/10 flex items-center justify-center text-[10px] font-black text-east-light border border-east-light/20 italic">{reg.profiles?.first_name?.[0]}{reg.profiles?.last_name?.[0]}</div>
                                                    <div>
                            ...
                            <div className="flex gap-4 pt-4">
                                <button onClick={handleSaveSession} className="flex-1 bg-east-light text-black font-black italic uppercase text-sm py-6 rounded-2xl hover:bg-white transition-all shadow-2xl active:scale-95 flex items-center justify-center gap-3"><Save size={20} /> Deploy Changes</button>

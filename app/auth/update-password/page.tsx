...
const InputField: React.FC<{
    ...
    placeholder: string;
    name?: string;
}> = ({ label, type, value, icon: Icon, onChange, placeholder, name }) => (
    <div className="relative mb-5 group">
        <label htmlFor={name || label} className="block text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-2 px-1 group-focus-within:text-east-light transition-colors">{label}</label>
            ...
            <div className="w-full max-w-sm bg-white/[0.03] backdrop-blur-xl rounded-[2.5rem] p-10 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] border border-white/10 animate-fadeIn relative z-10 overflow-hidden">
                ...
                {!success ? (
                    ...
                    <div className="text-center py-6">
                        ...
                        <h2 className="font-montserrat font-black italic text-3xl text-white tracking-tighter mb-4 uppercase leading-none">ALL SET</h2>
                        <p className="text-[10px] text-white/30 uppercase font-black tracking-widest leading-relaxed mb-10 px-4">Your password has been reset successfully. You can now access your account with the new credentials.</p>
                        ...
                        >
                            <div className="absolute inset-0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000 bg-gradient-to-r from-transparent via-white/50 to-transparent" />
                            <span className="relative z-10 uppercase tracking-tighter inline-flex items-center">
                                GO TO LOGIN <ArrowRight size={20} className="ml-2" />
                            </span>
                        </button>
                    </div>
                )}

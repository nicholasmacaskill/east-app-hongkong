...
import { ChevronLeft, Plus, Search, QrCode as QrIcon, Trash2, Edit2, Shield, Copy, X, User, Users, Star, Mail, AlertCircle, Coins, Calendar } from 'lucide-react';
...
export default function DirectoryPage() {
    ...
    const handleDeleteProfile = async (profileId: string, profileName: string) => {
        ...
        try {
            ...
            alert(`${profileName} has been deleted successfully`);
            fetchProfiles();
    ...
    return (
        <div className="flex flex-col gap-6 animate-fadeIn pb-20">
                ...
                <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
                    <div className="bg-[#1e1e1e] p-6 rounded-2xl w-full max-w-md border border-white/10 max-h-[90vh] overflow-y-auto">
                        <h2 className="font-black italic text-xl uppercase mb-4 text-[#28D160]">
                            {newUser.role === 'parent' ? 'Add New Household' : 'Add New Athlete'}
                        </h2>
            ...
            <div className="relative">
                ...
                <input
                    type="text"
                    placeholder="Search households, athletes, teams..."
                    value={searchTerm}
                    ...
                    <>
                                ...
                                {households.map(parent => {
                                    ...
                                    return (
                                        <div key={parent.id} className="bg-[#1e1e1e] rounded-3xl border border-white/5 overflow-hidden">
                                            {/* Parent Row */}
                                            <div className="p-5 flex items-center justify-between border-b border-white/5 bg-gradient-to-r from-purple-900/10 to-transparent">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-12 h-12 rounded-full bg-purple-600/20 border border-purple-600/30 flex items-center justify-center text-purple-400">
                                                        <User size={24} />
                                                    </div>
                                                    <div>
                                                        ...
                                                        {children.map(child => (
                                                            <div key={child.id} className="bg-[#252525]/50 rounded-2xl p-4 border border-white/5 flex items-center justify-between group">
                                                                ...
                                                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                    <button onClick={() => handleEditClick(child)} className="p-1.5 text-gray-500 hover:text-[#28D160]">
                                                                        <Edit2 size={14} />
                                                                    </button>
                                                                    <button onClick={() => handleDeleteProfile(child.id, `${child.first_name} ${child.last_name}`)} className="p-1.5 text-gray-500 hover:text-red-500">
                                                                        <Trash2 size={14} />
                                                                    </button>
                                                                </div>
                                                            </div>
                                            ...
                                            {unassignedPlayers.map(player => (
                                                <div key={player.id} className="bg-[#1e1e1e] rounded-2xl p-4 border border-amber-500/20 flex items-center justify-between group">
                                                    ...
                                                    <div className="flex gap-1">
                                                        <button onClick={() => handleEditClick(player)} className="p-1.5 text-gray-500 hover:text-[#28D160]">
                                                            <Edit2 size={14} />
                                                        </button>
                                                    </div>
                                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {coaches.map(coach => (
                                        <div key={coach.id} className="bg-[#1e1e1e] rounded-2xl p-4 border border-white/5 flex items-center justify-between group">
                                            <div className="flex items-center gap-3">
                                                <div className="w-12 h-12 rounded-full bg-[#28D160]/10 border border-[#28D160]/20 flex items-center justify-center text-[#28D160]">
                                                    <Star size={20} />
                                                </div>
                                                <div>
                                            ...
                                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => { setSelectedCoach(coach); setShowAvailability(true); }} className="p-1.5 text-gray-500 hover:text-blue-400" title="Manage Availability">
                                                    <Calendar size={14} />
                                                </button>
                                                <Link href={`/sys-admin/schedule?instructor=${encodeURIComponent(coach.first_name + ' ' + coach.last_name)}`} className="p-1.5 text-gray-500 hover:text-[#28D160]" title="Add Session">
                                                    <Plus size={14} />
                                                </Link>
                                                <button onClick={() => handleEditClick(coach)} className="p-1.5 text-gray-500 hover:text-[#28D160]">
                                                    <Edit2 size={14} />
                                                </button>
                                            </div>
                                        </div>
                        ...
                        {activeTab === 'admins' && (
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {admins.map(admin => (
                                        <div key={admin.id} className="bg-[#1e1e1e] rounded-2xl p-4 border border-white/5 flex items-center justify-between group">
                                            <div className="flex items-center gap-3">
                                                <div className="w-12 h-12 rounded-full bg-blue-600/10 border border-blue-600/20 flex items-center justify-center text-blue-400">
                                                    <Shield size={20} />
                                                </div>
                                                <div>
                                            ...
                                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => handleEditClick(admin)} className="p-1.5 text-gray-500 hover:text-[#28D160]">
                                                    <Edit2 size={14} />
                                                </button>
                                            </div>
                                        </div>
